...
public class Intake extends SubsystemBase {
  ...
  private final DCMotorSim axisMotorModel =
      new DCMotorSim(
          edu.wpi.first.math.system.plant.DCMotor.getKrakenX60(1), 1, Constants.jKgMetersSquared);
  ...
  public boolean isIntaking() {
    return intakeMotor.getVelocity().getValue()
        >= (FloorIntakeConstants.MotorRPM / 60) - FloorIntakeConstants.VelocityTolerance;
  }
  ...
  private void applyMotorConfigs(InvertedValue inverted) {
    ...
    if (!status.isOK()) {
      System.out.println(
          "Could not apply motor output configs to intake motor, with error code: "
              + status.toString());
    }
    ...
    if (!status.isOK()) {
      System.out.println(
          "Could not apply current limit configs to intake motor, with error code: "
              + status.toString());
    }
